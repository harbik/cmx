...
mod roundtrip_edge_cases_test {
    ...
    fn test_roundtrip_with_profile_id_flag() {
        ...
        assert_ne!(profile_id_after[15], 1, "Profile ID should not contain the flag after serialization");
        
        // The profile ID in bytes and after roundtrip should match
        assert_eq!(profile_id_in_bytes, profile_id_after, "Profile ID should be preserved in roundtrip");
    ...
    fn test_roundtrip_without_profile_id_flag() {
        ...
        // Should be all zeros
        assert!(profile_id_before.iter().all(|&x| x == 0), "Profile ID should be all zeros");
        ...
        assert!(profile_id_in_bytes.iter().all(|&x| x == 0), "Profile ID should remain all zeros in serialized bytes");
        ...
        // Should still be all zeros
        assert!(profile_id_after.iter().all(|&x| x == 0), "Profile ID should remain all zeros after roundtrip");
    ...
    fn test_real_profile_checksum_verification() {
        ...
        if test_profile_path.exists() {
            ...
            assert_eq!(calculated_profile_id, expected_checksum, 
