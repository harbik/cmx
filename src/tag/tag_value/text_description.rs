...
impl TextDescriptionTypeToml {
    ...
    fn from_bytes(buf: &[u8]) -> Result<Self, String> {
        ...
        cursor.seek(SeekFrom::Current(4)).map_err(|e| e.to_string())?; // Skip reserved
        ...
        cursor.read_exact(&mut ascii_bytes).map_err(|e| e.to_string())?;
        ...
        cursor.read_exact(&mut unicode_bytes).map_err(|e| e.to_string())?;
        ...
        cursor.read_exact(&mut script_buf).map_err(|e| e.to_string())?;
        ...
        cursor.read_exact(&mut mac_len_buf).map_err(|e| e.to_string())?;
...
/*
impl super::TextDescriptionType {
    pub fn get_ascii(&self) -> String {
        let text_desc = TextDescriptionToml::from_bytes(self.0.as_slice())
            .expect("Failed to parse textDescriptionType");
        text_desc.get_ascii()
    }

    pub fn get_unicode(&self) -> (String, Language) {
        let text_desc = TextDescriptionToml::from_bytes(self.0.as_slice())
            .expect("Failed to parse textDescriptionType");
        text_desc.get_unicode()
    }
}
 */