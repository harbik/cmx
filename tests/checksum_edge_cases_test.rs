...
mod checksum_edge_cases_test {
    use cmx::profile::{md5checksum, set_profile_id};
    ...
    fn test_md5checksum_preserves_original_data() {
        ...
        (84..=99).for_each(|i| {
            assert_eq!(test_data[i], 0, "Profile ID byte {i} not cleared");
        });

        // Verify checksum is not all zeros
        assert!(
            !checksum.iter().all(|&x| x == 0),
    ...
    fn test_set_profile_id_consistency() {
        ...
        assert_eq!(
            &test_data1[84..=99],
            &checksum1[..],
            "Profile ID should match checksum"
        );
        assert_eq!(
            &test_data2[84..=99],
            &checksum2[..],
