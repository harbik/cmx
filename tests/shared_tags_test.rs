...
mod shared_tags_test {
    ...
    fn test_shared_tags_detection() {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("icc") {
                if let Ok(original_bytes) = fs::read(&path) {
                    if let Ok(profile) = RawProfile::from_bytes(&original_bytes) {
                        ...
                        assert_eq!(
                            profile.uses_shared_tags(),
                            roundtrip_profile.uses_shared_tags(),
        ...
        println!(
            "Tested {profiles_tested} profiles: {shared_count} with shared tags, {non_shared_count} without"
        );
        assert!(
            profiles_tested > 0,
            "Should have tested at least one profile"
        );
    ...
    fn test_manual_shared_tags_control() {
        ...
        assert!(
            original_profile.uses_shared_tags(),
            "Default profile should use shared tags"
        );
